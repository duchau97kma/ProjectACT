    ...
    <div class="container">
        <div class="row">
            <div class="col-md-7 mb-5">
                <spring:form method="post" action="updateCart" class="p-5 bg-white">
                <table class="table table-bordered table-sm" style="text-align: center">
                    <th></th>
                    <th>Tên Tour</th>
                    <th>Ngày đi</th>
                    <th>Số người <input type="submit" value="update"></th>
                    <th>Giá gốc</th>
                    <th>Giá khuyến mại</th>
                    <c:set var="total" value="0"></c:set>
                    <c:forEach var="items" items="${shoppingCart}" varStatus="i">

                        <c:set var="total"
                               value="${total + (items.tour.price - (items.tour.price*items.tour.discount))}"></c:set>

                        <tr>
                            <td><a href="removeItem?id=${i.index}">Remove</a></td>
                            <td>${items.tour.nameTour}</td>
                            <td>${items.tour.departureDate}</td>
<%--                            <td><input min="1" max="${items.totalMember}" type="text" name="totalUser" value="${items.totalMember}" size="5"></td>--%>
                            <td><select name="totalUser" id="treatment" class="form-control">
                                <option value="${items.totalMember}">${items.totalMember}</option>
                                <c:forEach var="i" begin="1" end="${items.totalMember}">
                                <option value="${(items.totalMember+1)-i}">${(items.totalMember+1)-i}</option>
                                </c:forEach>
                            </select></td>
                            <td>${items.tour.price}</td>
                            <td>${items.tour.price - (items.tour.price*items.tour.discount)}</td>
                        </tr>
                    </c:forEach>
                        <tr>
        ...
        <spring:form method="post" action="checkoutCart">
                <input type="submit" value="Thanh toán" class="btn btn-primary py-2 px-4 text-white">
        </spring:form>
            </div>
            <div class="col-md-5">
                ...
                <div class="p-4 mb-3 bg-white">
                    ...
                    <p class="mb-4">141 Chiến Thắng, Tân Triều, Thanh Trì, Hà Nội</p>

                    <p class="mb-0 font-weight-bold">Phone</p>
                    <p class="mb-4"><a href="#">+84 33624 2559</a></p>
